...
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect","Detect_AFPN3"
...
class AFPN(nn.Module):
    def __init__(self, in_channels, out_channels):
        """修改后的AFPN初始化
        Args:
            in_channels: 输入特征图的通道数列表，如[256, 512, 1024]
            out_channels: 输出通道数
        """
...
class Detect_AFPN3(nn.Module):
    ...
    def __init__(self, nc=80, channel=256, in_channels=()):
        """修改后的检测头初始化
        Args:
            nc: 类别数
            channel: AFPN内部通道数
        ...
        super().__init__()
        self.nc = nc
        self.nl = len(in_channels)  # 特征图层级数
        self.reg_max = 16
        self.no = nc + self.reg_max * 4
        self.stride = torch.zeros(self.nl)

        # 使用实际输入通道数
        c2, c3 = max((16, in_channels[0] // 4, self.reg_max * 4)), max(in_channels[0], min(self.nc, 100))

        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(channel, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1))
            for _ in range(self.nl))

        self.cv3 = nn.ModuleList(
            nn.Sequential(Conv(channel, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1))
            for _ in range(self.nl))

        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
        self.AFPN = AFPN(in_channels, channel)
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.stem = nn.ModuleList(nn.Sequential(PConv(x, x, 1), Conv(x, x, 1)) for x in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
